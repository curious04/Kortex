...
3. Add environment variables in Vercel project settings:
   ...
   - `RESEND_API_KEY` (optional) — enables emailing new owners an invite when you add them from the UI, and emailing owners whenever a contributor submits a note via Pull Request. Free tier at https://resend.com — sign up, then go to **API Keys** in the dashboard and create one (starts with `re_`). Without a verified sending domain, Resend's sandbox mode only delivers to the email on your Resend account — verify a domain to email arbitrary people.
   - `RESEND_FROM` (optional) — the "from" address for these emails, e.g. `Kortex <notify@yourdomain.com>`. Defaults to Resend's shared sandbox address.
   - `KORTEX_NOTIFY_EMAIL` (optional) — comma-separated email address(es) that always get the "new PR submitted" email, regardless of whether any owner has a public GitHub email set. Example: `KORTEX_NOTIFY_EMAIL=me@example.com`.
